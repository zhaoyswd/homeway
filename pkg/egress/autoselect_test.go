...
// 候选枚举只做「结构性过滤」，能不能出网由探针决定 —— 这里用真机网卡跑一遍结构判据。
...
// 用注入的 prober 覆盖判定逻辑（真机探针在网络沙箱里不稳定，逻辑必须能在本地钉住）。
...
func TestSelectBestAllFail(t *testing.T) {
	...
	var logged string
	_, err := selectBestWith(context.Background(), cands, nil, nil, time.Second,
		func(f string, a ...any) { logged = f + strings.Repeat(" ", 1) }, probe)
	if err == nil {
		t.Fatal("全不通应报错")
	}
	if !strings.Contains(err.Error(), "都探不通") {
		t.Fatalf("错误信息应说清全部探不通：%v", err)
	}
	_ = logged
}
